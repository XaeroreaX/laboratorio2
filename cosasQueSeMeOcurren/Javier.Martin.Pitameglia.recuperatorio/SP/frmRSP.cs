...
namespace SP
{
    ...
    public partial class frmRSP : Form
    {
        ...
        private void btnPunto4_Click(object sender, EventArgs e)
        {
            ...
            serializar = dh.guardar("DepositoHeredado.xml");
            ...
            deserializar = ((Interface2)dh).leer("DepositoHeredado.xml", out aux.deposito);
            aux.productos = aux.deposito.productos;
...
/*
            try
            {
                Burbujeo.GuardarArchivo(DateTime.Now + "   Pase por el try\n");
                Burbujeo.MetodoClase();
            }
            catch (MiException ex)
            {
                //Implementar escribir en archivo de texto
                Burbujeo.GuardarArchivo(DateTime.Now + "   " + ex.Message);
                Burbujeo.GuardarArchivo(DateTime.Now + "   Pase por el catch de MiException\n");
            }
            finally
            {
                //Implementar leer en archivo de texto
                Burbujeo.GuardarArchivo(DateTime.Now + "   Pase por el finally\n");
